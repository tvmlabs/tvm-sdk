...
{% hint style="warning" %}
Not all transactions that are successful are valid transfers and not all transactions that are aborted actually failed. [Read below how to understand which transfers are successful transfers and which are not](add\_to\_backend.md#how-to-determine-a-successful-transaction-1).
{% endhint %}
