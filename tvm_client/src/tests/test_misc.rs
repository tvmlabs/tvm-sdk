...
fn test_contexts() {
    ...
    let foo_context = foo
        .request_json("context.get", TestClient::missing_params())
        .unwrap()
        .as_u64()
        .unwrap();
    let bar_context = bar
...
fn test_tonos_key_derivation() {
    ...
    assert_eq!(
        TestClient::get_map_string(&keys, "public"),
        "f4c8c7eb4f81f1153703b76a23827f370a7a776da6f7f74b2fd692a5bc4edab3"
    );
    assert_eq!(
        TestClient::get_map_string(&keys, "secret"),
        "eab2364790e3d8629f4ed019148d373cd6acc5b7af3ec2ada35c591c5de48fec"
    );

    // Surf Style TONOS Key Derivation
    let hdk_master = TestClient::parse_string(client.request(
    ...
    let hdk_root = TestClient::parse_string(client.request(
    ...
    let secret = TestClient::parse_string(client.request(
...
fn test_tg_mnemonic() {
    ...
    assert_eq!(
        TestClient::get_map_string(&keys, "public"),
        "36cf573461971289477d65ebc67664805207d07d1cfb337bc28ff86a3f4fa8c7"
    );
    ...
    let tvm_public = TestClient::parse_string(client.request(
        "crypto.tvm_public_key_string",
    ...
    let phrase = TestClient::parse_string(client.request(
    ...
    let mnemonic = TestClient::parse_string(client.request(
    ...
    let public = TestClient::get_map_string(
        &TestClient::parse_object(client.request(
            "crypto.mnemonic.derive.sign.keys",
            json!({
                "phrase": mnemonic,
                "dictionary": 0,
    ...
    let tvm_public = TestClient::parse_string(
        client.request("crypto.tvm_public_key_string", Value::String(public)),
    );
    assert_eq!(
        tvm_public,
    ...
    //    let tvm_phrase = "shove often foil innocent soft slim pioneer day uncle drop nephew soccer worry renew public hand word nut again dry first delay first maple";
...
fn test_wallet_deploy() {
    ...
    let keys = client
        .request_json("crypto.ed25519.keypair", json!({}))
        .unwrap();
    ...
    let deployed = client.request_map(
        "contracts.deploy",
        json!({
            "abi": abi.clone(),
            "constructorParams": json!({}),
...
fn test_address_parsing() {
    ...
    let address =
        tvm_block::MsgAddressInt::with_standart(None, -1, hex::decode(short).unwrap().into())
            .unwrap();
    let wc0_address =
        tvm_block::MsgAddressInt::with_standart(None, 0, hex::decode(short).unwrap().into())
            .unwrap();
...
fn test_api_reference() {
    let client = TestClient::new();
    let api = serde_json::from_value::<API>(
        client
            .request_json("config.get_api_reference", Value::Null)
            .unwrap(),
    )
    .unwrap();
    let m = api
        .methods
        .iter()
        .find(|x| x.name == "crypto.tvm_crc16")
        .unwrap();
