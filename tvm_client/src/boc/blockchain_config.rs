...
pub fn get_blockchain_config(
    ...
    let bytes = tvm_block::boc::write_boc(&cell)
        .map_err(|err| Error::serialization_error(err, "config cells to bytes"))?;

    Ok(ResultOfGetBlockchainConfig {
        config_boc: tvm_block::base64_encode(&bytes),
    })
}

pub(crate) fn extract_config_from_block(block: &tvm_block::Block) -> ClientResult<tvm_block::ConfigParams> {
    let extra = block
        .read_extra()
...
pub(crate) fn extract_config_from_zerostate(zerostate: &tvm_block::ShardStateUnsplit) -> ClientResult<tvm_block::ConfigParams> {
