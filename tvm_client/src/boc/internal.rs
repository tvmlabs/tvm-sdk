...
pub(crate) fn get_boc_hash(boc: &[u8]) -> ClientResult<String> {
    let cells =
        tvm_block::boc::read_single_root_boc(&boc).map_err(|err| Error::invalid_boc(err))?;
...
pub fn serialize_cell_to_bytes(cell: &tvm_block::Cell, name: &str) -> ClientResult<Vec<u8>> {
    tvm_block::boc::write_boc(&cell)
        .map_err(|err| Error::serialization_error(err, name))
}

pub fn serialize_cell_to_base64(cell: &tvm_block::Cell, name: &str) -> ClientResult<String> {
    Ok(tvm_block::base64_encode(&serialize_cell_to_bytes(cell, name)?))
}
...
pub fn serialize_cell_to_boc(
    context: &ClientContext, cell: tvm_block::Cell, name: &str, boc_cache: Option<BocCacheType>,
) -> ClientResult<String> {
