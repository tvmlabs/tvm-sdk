...
fn parse_account() {
    ...
    assert_eq!(
        result.parsed["id"],
        "0:0adc58595fd7acf3c4703aa3db2a3016ae140e7d0d426947eb9ba1b793819b1b"
    );
    assert_eq!(result.parsed["last_trans_lt"], "0x284");
    assert_eq!(result.parsed["balance"], "0x92a588b8fbd");
}
...
fn parse_pruned_account() {
    ...
    assert_eq!(
        ...
        "1bd39e6737431df9c01b691c328d147209cd023d790f747ff7e2a26e724fc052"
    );
    assert!(result.parsed["code"].is_null());
    assert_eq!(
        result.parsed["code_hash"],
        "f67d2692febb6084a0baf4f273846eca0aa1a5ee600acd3263aa470de5d54cac"
    );
}
