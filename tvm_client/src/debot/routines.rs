...
pub async fn call_routine(
    ...
    match name {
        ...
        "encryptAuth" => {
            ...
            Ok(json!({
                "encrypted": hex::encode(tvm_block::base64_decode(&encrypted).unwrap())
            }))
        }
...
pub(super) fn load_boc_from_file(_ton: TonClient, arg: &str) -> Result<String, String> {
    ...
    Ok(tvm_block::base64_encode(&boc))
}
...
pub(super) async fn sign_hash(
    ...
    let result = signing_box_sign(
        ...
        ParamsOfSigningBoxSign {
            unsigned: tvm_block::base64_encode(&hash_as_bigint.to_bytes_be().1),
            signing_box: signer,
        },
    )
    .await
...
pub(super) fn nacl_box(ton: TonClient, args: serde_json::Value) -> Result<String, String> {
    ...
    let result = crate::crypto::nacl_box(
        ton,
        ParamsOfNaclBox {
            decrypted: tvm_block::base64_encode(&get_arg(&args, "decrypted")?),
