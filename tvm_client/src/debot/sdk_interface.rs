...
impl SdkInterface {
    ...
    fn get_random(&self, args: &Value) -> InterfaceResult {
        ...
        let buf = tvm_block::base64_decode(&rnd)
    ...
    fn chacha20(&self, args: &Value) -> InterfaceResult {
        let answer_id = decode_answer_id(args)?;
        let data = tvm_block::base64_encode(&hex::decode(&get_arg(args, "data")?).unwrap());
        ...
        Ok((
            answer_id,
            json!({ "output": hex::encode(&tvm_block::base64_decode(&result.data).unwrap()) }),
        ))
    }
    ...
    fn nacl_box(&self, args: &Value) -> InterfaceResult {
        let answer_id = decode_answer_id(args)?;
        let decrypted = tvm_block::base64_encode(
        ...
        Ok((
            answer_id,
            json!({ "encrypted": hex::encode(&tvm_block::base64_decode(&result.encrypted).map_err(|e| format!("{}", e))?) }),
        ))
    }

    fn nacl_box_open(&self, args: &Value) -> InterfaceResult {
        let answer_id = decode_answer_id(args)?;
        let encrypted = tvm_block::base64_encode(
        ...
        Ok((
            answer_id,
            json!({ "decrypted": hex::encode(&tvm_block::base64_decode(&result.decrypted).map_err(|e| format!("{}", e))?) }),
        ))
    }
    ...
    async fn encrypt_or_decrypt(&self, args: &Value, encrypt: bool) -> InterfaceResult {
        ...
        let encryption_box = EncryptionBoxHandle(get_num_arg::<u32>(args, "boxHandle")?);
        let data =
            tvm_block::base64_encode(&hex::decode(&get_arg(args, "data")?).map_err(|e| format!("{}", e))?);
        ...
        let (result, data) = match result {
            Ok(data) => {
                let data = tvm_block::base64_decode(&data)
                    .map(|x| hex::encode(x))
    ...
    async fn sign_hash(&self, args: &Value) -> InterfaceResult {
        ...
        let signature = signing_box_sign(
            ...
            ParamsOfSigningBoxSign {
                ...
                unsigned: tvm_block::base64_encode(sign_hash.as_slice()),
            },
        )
        .await
        .map_err(|e| format!("{}", e))?
