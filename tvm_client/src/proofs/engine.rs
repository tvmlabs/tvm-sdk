...
impl ProofHelperEngineImpl {
    ...
    pub(crate) async fn add_mc_blocks_file_hashes(
        ...
        while !proofs_sorted.is_empty() {
            ...
            for (exp_seq_no, exp_val) in expected.iter_mut().take(blocks.len()) {
                let (seq_no, mut block) = blocks.remove(0);

                let expected_seq_no = *exp_seq_no + 1;
                ...
                exp_val["file_hash"] = block["prev_ref"]["file_hash"].take();
            }
    ...
    pub(crate) async fn query_shard_block_bocs(
        ...
        let mut result = Vec::with_capacity(blocks.len());
        for (i, (seq_no, block)) in blocks.iter().enumerate() {
