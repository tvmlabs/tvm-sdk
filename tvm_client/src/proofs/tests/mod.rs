...
async fn test_query_shard_block_bocs() -> Result<()> {
    ...
    assert_eq!(
        bocs[0],
        tvm_block::base64_decode(SHARD_BLOCK_0_A000000000000000_99_BOC)?
    );
    assert_eq!(
        bocs[2],
        tvm_block::base64_decode(SHARD_BLOCK_0_A000000000000000_101_BOC)?
    );
...
async fn test_check_shard_block() -> Result<()> {
    ...
    let boc_99 = tvm_block::base64_decode(SHARD_BLOCK_0_A000000000000000_99_BOC)?;
    engine.check_shard_block(&boc_99).await?;

    let boc_101 = tvm_block::base64_decode(SHARD_BLOCK_0_A000000000000000_101_BOC)?;
...
async fn test_transaction_get_required_data() -> Result<()> {
    ...
    async fn test(engine: &ProofHelperEngineImpl, transaction_json: Value) -> Result<()> {
        ...
        assert_eq!(boc, tvm_block::base64_decode(BOC)?);
...
async fn test_message_get_required_data() -> Result<()> {
    async fn test(
        ...
        assert_eq!(boc, tvm_block::base64_decode(message_boc)?);
        assert_eq!(
