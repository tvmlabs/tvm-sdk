...
async fn test_calc_storage_fee() {
    ...
    let result: ResultOfCalcStorageFee = client.request_async(
        ...
        ParamsOfCalcStorageFee {
            account: tvm_block::base64_encode(&include_bytes!("../boc/test_data/account.boc")),
            period: 1000,
        }
    ).await.unwrap();
...
fn test_compression() {
    ...
    let compressed: ResultOfCompressZstd = client.request(
        ...
        ParamsOfCompressZstd {
            uncompressed: tvm_block::base64_encode(uncompressed),
            level: Some(21),
        }
    ).unwrap();

    assert_ne!(tvm_block::base64_decode(&compressed.compressed).unwrap(), uncompressed);
    ...
    let decompressed = tvm_block::base64_decode(&decompressed.decompressed).unwrap();
