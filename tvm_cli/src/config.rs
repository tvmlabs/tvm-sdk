...
impl FullConfig {
    fn new(config: Config, path: String) -> Self {
        FullConfig { config, endpoints_map: Self::default_map(), aliases: BTreeMap::new(), path }
    }
    ...
    pub fn from_file(path: &str) -> FullConfig {
        ...
        match serde_json::from_str(&conf_str) {
            Ok(config) if config != Config::default() => FullConfig::new(config, path.to_string()),
            _ => {
                let mut full_config = match serde_json::from_str(&conf_str) {
                    Ok(cfg) => cfg,
                    _ => {
                        let conf_str =
                            std::fs::read_to_string(global_config_path()).ok().unwrap_or_default();
