...
fn reduce_type(ty: &Type, module: &Module, api: &API) -> Type {
    match ty {
        Type::Array { item } => Type::Array { item: Box::new(reduce_type(item, module, api)) },
        Type::Optional { inner } => {
            Type::Optional { inner: Box::new(reduce_type(inner, module, api)) }
        }
        ...
        Type::Struct { fields } => {
            if fields.len() == 1 && fields[0].name.is_empty() {
                ...
                Type::Struct {
                    fields: fields.iter().map(|x| reduce_field(x, module, api)).collect(),
                }
            }
        }
        Type::EnumOfTypes { types } => {
            ...
            Type::EnumOfTypes { types: reduced_types }
        }
        _ => ty.clone(),
    }
}
...
fn reduce_function(function: &Function, module: &Module, api: &API) -> Function {
    Function {
        ...
        params: function.params.iter().map(|p| reduce_field(p, module, api)).collect(),
...
fn reduce_module(module: &Module, api: &API) -> Module {
    Module {
        ...
        types: module.types.iter().map(|t| reduce_field(t, module, api)).collect(),
        functions: module.functions.iter().map(|f| reduce_function(f, module, api)).collect(),
...
fn write_text_to_out_dir(text: String, out_dir: String) -> CliResult<()> {
    let out_dir = if let Some(stripped) = out_dir.strip_prefix("~/") {
        dirs::home_dir().ok_or(CliError::with_message("Home dir not found".into()))?.join(stripped)
    } else {
        out_dir.into()
    };
