...
pub async fn print_account_data(
    ...
    let (si, code_hash) = match state_init {
        Some(state_init) => {
            let code = state_init.code.clone().ok_or("failed to obtain code from the StateInit")?;
...
async fn decode_body(
    ...
    let (mut res, is_external) = {
        match decode_msg_body(ton.clone(), abi_path, body_base64, false, config).await {
            Ok(res) => (res, true),
            Err(_) => {
                (decode_msg_body(ton.clone(), abi_path, body_base64, true, config).await?, false)
            }
        }
    };
...
pub mod msg_printer {
    ...
    async fn get_code_version(ton: TonClient, code: String) -> String {
        let result =
            get_compiler_version(ton, ParamsOfGetCompilerVersion { code });
