...
fn generate_public_key(seed: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let out =
        cmd.arg("genpubkey").arg(seed).output().expect("Failed to generate a public key phrase.");
...
fn test_deploy_alias() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("--config")
        ...
        .arg("--alias")
        .arg(alias);
    cmd.assert().success().stdout(predicate::str::contains(address.clone()));
...
fn test_config_aliases() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("--config").arg(config_path).arg("config").arg("alias").arg("reset");
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("--config").arg(config_path).arg("config").arg("alias").arg("remove").arg("msig");
    cmd.assert().success().stdout(predicate::str::contains("{}"));
...
fn test_config_endpoints() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert().success().stdout(predicate::str::contains(r#""url": "myownhost","#)).stdout(
...
fn test_genaddr() -> Result<(), Box<dyn std::error::Error>> {
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("genaddr").arg("tests/samples/wallet.tvc").arg("--genkey").arg("/dev/null");
    cmd.assert()
        .success()
...
fn test_async_deploy() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("--config")
        ...
        .arg("--sign")
        .arg(key_path);
    cmd.assert().success().stdout(predicate::str::contains(addr.clone()));
...
fn test_genaddr_update_key() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("genphrase").arg("--dump").arg(key_path).assert().success();
...
fn test_genaddr_seed() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    let out = cmd.arg("genaddr").arg("--setkey").arg(key_path).arg(SAFEMSIG_TVC).assert().success();

    let msig_addr = grep_address(&out.get_output().stdout);

    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    let out = cmd.arg("genaddr").arg("--setkey").arg(seed).arg(SAFEMSIG_TVC).assert().success();
...
fn test_nodeid() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("nodeid").arg("--keypair").arg("tests/samples/exp.json");
    cmd.assert().success().stdout(predicate::str::contains(
...
fn test_override_config_path() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.env("TONOSCLI_CONFIG", "./tests/conf2.json")
        .arg("account")
        .arg("0:841288ed3b55d9cdafa806807f02a0ae0c169aa5edfe88a789a6482429756a94");
    cmd.assert().failure().stdout(predicate::str::contains("Url: https://test2.ton.dev"));
...
fn test_global_config() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("--config").arg(config_path).arg("config").arg("--is_json").arg("true");
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("url\": \"net.evercloud.dev"))
        .stdout(predicate::str::contains("addr\": null"))
...
fn test_old_config() -> Result<(), Box<dyn std::error::Error>> {
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("--config").arg("tests/rfld.conf.json").arg("config").arg("--list");
    cmd.assert().success().stdout(predicate::str::contains("url\": \"rfld-dapp.itgold.io"));

    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("--config").arg("tests/test.conf.json").arg("config").arg("--list");
    cmd.assert().success().stdout(predicate::str::contains("url\": \"http://127.0.0.1/"));
    Ok(())
}
...
fn test_sendfile() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("--url").arg(&*NETWORK).arg("--config").arg(config_path).arg("sendfile").arg(msg_path);
    cmd.assert().success();
...
fn test_account_command() -> Result<(), Box<dyn std::error::Error>> {
    ...
    set_config(&["--url", "--addr"], &[&*NETWORK, GIVER_V2_ADDR], Some(config_path))?;
    ...
    set_config(&["--url", "--addr"], &[&*NETWORK, "tests/account.boc"], Some(config_path))?;

    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("--config").arg(config_path).arg("account").arg("--boc");
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("acc_type:      Active"))
        .stdout(predicate::str::contains("balance:"))
...
fn test_decode_body_constructor_for_minus_workchain() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert().success().stdout(predicate::str::contains("constructor: {")).stdout(
        predicate::str::contains(
    ...
    cmd.arg("--config")
        ...
        .arg("--abi")
        .arg("tests/samples/Subscription.abi.json");
    cmd.assert().failure().stdout(predicate::str::contains("body is not a valid base64 string"));
    ...
    cmd.arg("--config")
        ...
        .arg("--abi")
        .arg("tests/samples/Subscription.abi.json");
    cmd.assert().failure().stdout(predicate::str::contains("failed to decode body"));
...
fn test_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("call")
        ...
        .arg("error")
        .arg(r#"{"code":101}"#);
    cmd.assert().stdout(predicate::str::contains(r#""exit_code": 101,"#));

    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("call").arg("--abi").arg(DEPOOL_ABI).arg(&depool_addr).arg("outOfGas").arg(r#"{}"#);
    cmd.assert().stdout(predicate::str::contains(r#""exit_code": -14,"#));
...
fn test_depool_body() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("body").arg("--abi").arg(DEPOOL_ABI).arg("addOrdinaryStake").arg(r#"{"stake":65535}"#);
    cmd.assert().success().stdout(predicate::str::contains("te6ccgEBAQEADgAAGAqsGP0AAAAAAAD//w=="));

    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("run").arg("--abi").arg(DEPOOL_ABI).arg(&depool_addr).arg("getData").arg("{}");
    cmd.assert().success().stdout(predicate::str::contains(r#"stake": "0"#));
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("run").arg("--abi").arg(DEPOOL_ABI).arg(&depool_addr).arg("getData").arg("{}");
    cmd.assert().success().stdout(predicate::str::contains(r#"stake": "65535"#));
...
fn test_depool_1() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("depool")
        ...
        .arg("--sign")
        .arg(SAFEMSIG_SEED);
    cmd.assert().success().stdout(predicate::str::contains(r#"Succeeded."#));

    sleep(Duration::new(1, 0));

    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("run").arg("--abi").arg(DEPOOL_ABI).arg(&depool_addr).arg("getData").arg("{}");
    cmd.assert().success().stdout(predicate::str::contains(r#"value": "2000000000"#));
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("run").arg("--abi").arg(DEPOOL_ABI).arg(&depool_addr).arg("getData").arg("{}");
    cmd.assert().success().stdout(predicate::str::contains(r#"value": "1000000000"#));

    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("depool").arg("--addr").arg(&depool_addr).arg("events");
    cmd.assert()
        .success()
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("depool").arg("--addr").arg(&depool_addr).arg("events").arg("-w");
    cmd.assert().success();
...
fn test_depool_2() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("--config")
        ...
        .arg(SAFEMSIG_SEED)
        .arg("--wait-answer");
    cmd.assert().success().stdout(predicate::str::contains(r#"Answer status: SUCCESS"#));
...
fn test_run_async_call() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("--config")
        ...
        .arg("getParameters")
        .arg("{}");
    cmd.assert().success().stdout(predicate::str::contains(r#""maxQueuedTransactions": "5","#));
...
fn test_alternative_syntax() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("--config").arg(config_path).arg("runx").arg("-m").arg("getParameters");
    cmd.assert().success().stdout(predicate::str::contains("Succeeded."));
...
fn test_options_priority() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("--config")
        ...
        .arg("getParameters")
        .arg("{}");
    cmd.assert().success().stdout(predicate::str::contains("maxQueuedTransactions\": \"5"));
    ...
    let params =
        format!("{{\"dest\":\"{}\",\"value\":1000000000,\"bounce\":false}}", address.clone());
    let mut cmd = Command::cargo_bin(BIN_NAME)?;
    cmd.arg("--config")
        .arg(config_path)
        .arg("call")
...
fn test_alternative_paths() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let message =
        data["Message"].to_string().trim_end_matches('\"').trim_start_matches('\"').to_string();
    ...
    cmd.arg("debug")
        ...
        .arg(SAFEMSIG_TVC)
        .arg(SAFEMSIG_CONSTR_ARG);
    cmd.assert().success().stdout(predicate::str::contains("Log saved to "));
