...
pub(crate) fn generate_internal_message(
    ...
    if let Some(value) = args.message_ecc.as_ref() {
        ...
        for (k, v) in ecc_map {
            let key = k.parse::<u32>()?;
            let value_str = v.as_str()
                .ok_or(anyhow::format_err!("Failed to parse ecc value"))?;
            let value = VarUInteger32::from_str(value_str)
                .map_err(|e| anyhow::format_err!("Failed to decode ecc value: {e}"))?;
            ecc.set(&key, &value)
                .map_err(|e| anyhow::format_err!("Failed to set ecc key: {e}"))?;
        }
    }
...
fn test_encode_body() -> anyhow::Result<()> {
    ...
    let _body = generate_message_body(&args)?;
    args.internal = true;
    args.address = Some("0:1111111111111111111111111111111111111111111111111111111111111111".to_string());
    let _body = generate_message_body(&args)?;
    Ok(())
}
...
fn test_generate_message() -> anyhow::Result<()> {
    ...
    args.internal = true;
    args.address = Some("0:1111111111111111111111111111111111111111111111111111111111111111".to_string());
    args.message_source = Some("0:1111111111111111111111111111111111111111111111111111111111111122".to_string());
