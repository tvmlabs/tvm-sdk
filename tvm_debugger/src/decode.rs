...
use tvm_block::{CommonMsgInfo, CurrencyCollection, Deserializable, Grams, Message, MsgAddressInt, OutAction, OutActions, Serializable, StateInit};
use tvm_types::{BuilderData, Cell, SliceData, write_boc};
...
pub(crate) fn decode_body(
    ...
    let res = decode_function_response(
        &load_abi_as_string(abi_file)?,
        function,
        body,
        internal,
        false
        ...
        .map_err(|e| anyhow::format_err!(
            "Failed to decode function response: {e}"
        ))?;
    println!("{res}");
    Ok(())
}

pub(crate) fn decode_actions(actions: StackItem, state: &mut StateInit, args: &Args) -> anyhow::Result<()> {
    let abi_file = args.abi_file.as_ref();
    let function_name = args.function_name.as_ref();
    let address = args.address.as_ref().map(|s| MsgAddressInt::from_str(s).unwrap()).unwrap_or_default();
                ...
                OutAction::SendMsg{mode: _, mut out_msg } => {
                    ...
                    if let Some(b) = out_msg.body() {
                        if abi_file.is_some() && function_name.is_some() {
                            decode_body(abi_file.unwrap(), function_name.unwrap(), b, out_msg.is_internal())?;
                        }
                    }
                },
...
pub fn msg_printer(msg: &Message) -> anyhow::Result<String> {
    ...
    let bytes = write_boc(&b.into_cell().map_err(|e| anyhow::format_err!("{e}"))?).map_err(|e| anyhow::format_err!("{e}"))?;
