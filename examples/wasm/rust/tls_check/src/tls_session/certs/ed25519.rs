...
// pub fn generate_key<R: RngCore + CryptoRng>(rng: &mut R) ->
// Result<(PublicKey, PrivateKey), Box<dyn Error>> { let mut seed = vec![0u8;
// SEED_SIZE]; rng.fill_bytes(&mut seed);
...
// fn new_key_from_seed_impl(private_key: &mut [u8], seed: &[u8]) -> Result<(),
// Box<dyn Error>> { let hash = Sha512::digest(seed);
...
// fn verify_inner(publicKey PublicKey, message, sig []byte, domPrefix, context
// string) -> bool { if l := len(publicKey); l != PublicKeySize {
