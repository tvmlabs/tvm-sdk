...
// u32::from(b[3]) | (u32::from(b[2]) << 8) | (u32::from(b[1]) << 16) |
// (u32::from(b[0]) << 24)
//}
...
fn gcm_add(x: &GcmFieldElement, y: &GcmFieldElement) -> GcmFieldElement {
    ...
    GcmFieldElement { low: x.low ^ y.low, high: x.high ^ y.high }
}
...
fn gcm_double(x: &GcmFieldElement) -> GcmFieldElement {
    ...
    let mut double = GcmFieldElement { high: x.high >> 1, low: x.low >> 1 };
...
impl Gcm {
    ...
    pub fn new_gcm_with_nonce_and_tag_size(
        ...
        let mut g = Gcm { cipher, nonce_size, tag_size, product_table: Default::default() };
    ...
    fn auth(
        &self,
        out: &mut [u8; GCM_TAG_SIZE],
        ciphertext: &[u8],
        additional_data: &[u8],
        tag_mask: &[u8; GCM_TAG_SIZE],
    ) {
        let mut y = GcmFieldElement { low: 0, high: 0 };
