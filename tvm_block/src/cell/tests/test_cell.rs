// Copyright (C) 2019-2022 TON. All Rights Reserved.
//
...
fn test_cell_state() {
    ...
    let c0 = BuilderData::with_raw_and_refs(vec![77], 1, vec![c1.clone(), c1, c2])
        .unwrap()
        .into_cell()
        .unwrap();
...
fn test_cell_data_serialization() {
    let initial_cell = CellData::with_params(
        ...
        Some([
            UInt256::from_slice(&[
                0x7a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf, 0x02,
                0x9c, 0x89, 0xfc, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d,
                0x5c, 0xf5, 0x92, 0x5b,
            ]),
            UInt256::from_slice(&[
                0x8a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf, 0x02,
                0x9c, 0x89, 0x00, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d,
                0x5c, 0x11, 0x92, 0x5b,
            ]),
            UInt256::default(),
            UInt256::default(),
    ...
    let serialized: Vec<u8> = vec![
        // cell type
        0x01, // bit length (tag is not included)
        0x1d, 0x00, // cell data
        0x01, 0x02, 0x03, 0x84, // level mask
        0x01, // store_hashes
        0x00, // hashes are stored
        0x01, // hashes count
        0x02, // hashes
        0x7a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf, 0x02, 0x9c,
        0x89, 0xfc, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d, 0x5c, 0xf5,
        0x92, 0x5b, 0x8a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf,
        0x02, 0x9c, 0x89, 0x00, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d,
        0x5c, 0x11, 0x92, 0x5b, // depths are stored
        0x01, // depths count
        0x02, // depths
        0x03, 0x00, 0x02, 0x00, // refs count
        0x02,
    ];
...
fn test_cell_data_serialization_2() {
    let cell = DataCell::with_params(
        ...
        &[
            0x1, 0x1, 0x7a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf,
            0x02, 0x9c, 0x89, 0xfc, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab,
            0x7d, 0x5c, 0xf5, 0x92, 0x5b, 0x1, 0x0, 0x80,
        ],
    ...
    let serialized: Vec<u8> = vec![
        ...
        0x02, // bitlen
        0x20, 0x01, // data
        0x01, 0x01, 0x7a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf,
        0x02, 0x9c, 0x89, 0xfc, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d,
        0x5c, 0xf5, 0x92, 0x5b, 0x01, 0x00, 0x00, // compatibility bite
        // level mask
        0x01, // store_hashes
        0x00, // hashes are stored
        0x01, // hashes count
        0x01, // hash
        0xcd, 0xd9, 0x93, 0xaf, 0xdc, 0xba, 0x18, 0x49, 0xb4, 0x61, 0xf7, 0x5d, 0x8d, 0x5f, 0xd3,
        0x74, 0xe6, 0x1d, 0x5c, 0x23, 0xd6, 0x3c, 0xfd, 0xc9, 0xe4, 0xd9, 0x31, 0xb7, 0x95, 0xea,
        0xf4, 0x5b, // depths are stored
        0x01, // depths count
        0x01, // depths
        0x00, 0x00, // refs count
        0x00,
    ];
...
fn test_cell_data_serialization_3() {
    let data = [
        ...
        0x7a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf, 0x02, 0x9c,
        0x89, 0xfc, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d, 0x5c, 0xf5,
        0x92, 0x5b, 0x8a, 0x86, 0x6c, 0x98, 0xfb, 0x11, 0x0b, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf,
        0x02, 0x9c, 0x89, 0xfc, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d,
        0x5c, 0xf5, 0x92, 0x5b, // 2 depths
        0, 1, 0, 1, 0x80,
    ];
    dbg!(data.len());
    ...
    let serialized: Vec<u8> = vec![
        ...
        0x02, 0x9c, 0x89, 0xfc, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d,
        0x5c, 0xf5, 0x92, 0x5b, // 2 depths
        0x0, 0x1, 0x0, 0x1, 0x00, // compatibility bite
        // level mask
        0b011, // store hashes
        0x0, 0x1, // hashes count
        0x1, // hash
        0x0b, 0x37, 0x98, 0x44, 0x5a, 0x06, 0x01, 0xce, 0x08, 0xdc, 0xff, 0x85, 0xb5, 0x95, 0x2f,
        0x80, 0xea, 0x8b, 0xa7, 0xfc, 0x80, 0x22, 0x81, 0xb6, 0x33, 0x77, 0x49, 0xe4, 0x8c, 0x8b,
        0x88, 0x02, // store depths
        0x1, 0x1, 0x0, 0x0, // refs
        0x0,
    ];
...
fn test_cell_data_serialization_4() {
    let initial_cell = CellData::with_params(
        ...
        Some([
            ...
            UInt256::from_slice(&[
                0x9a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf, 0x02,
                0x9c, 0x89, 0x00, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d,
                0x5c, 0x11, 0x92, 0x5b,
            ]),
            UInt256::default(),
    ...
    let serialized: Vec<u8> = vec![
        ...
        0x92, 0x5b, 0x8a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf,
        0x02, 0x9c, 0x89, 0xfc, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d,
        0x5c, 0xf5, 0x92, 0x5b, 0x9a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0,
        0x2f, 0xdf, 0x02, 0x9c, 0x89, 0x00, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c,
        0xab, 0x7d, 0x5c, 0x11, 0x92, 0x5b, // depths are stored
        0x01, // depths count
        0x03, // depths
        0x03, 0x00, 0x02, 0x00, 0x00, 0x00, // refs count
        0x02,
    ];
...
fn test_cell_data_serialization_5() {
    ...
    let serialized: Vec<u8> = vec![
        ...
        0x7a, 0x86, 0x6c, 0x58, 0xfb, 0x11, 0xab, 0xf4, 0x6c, 0xca, 0xf0, 0x2f, 0xdf, 0x02, 0x9c,
        0x89, 0xfc, 0x24, 0xf6, 0x4d, 0x76, 0x07, 0x4a, 0x58, 0x0e, 0x0c, 0xab, 0x7d, 0x5c, 0xf5,
        0x92, 0x5b, // depths are stored
        0x01, // depths count
        0x01, // depths
        0x21, 0xf1, // refs count
        0x02,
    ];
...
fn test_big_cell_creation() {
    ...
    fn check_big_cell(cell: &Cell, data: &[u8]) -> Result<()> {
        ...
        if cell.data() != data {
            fail!(
                "Cell data [{}] is not equal to original data [{}]",
    ...
    let hashes =
        Some([UInt256::rand(), UInt256::default(), UInt256::default(), UInt256::default()]);
    let depths = Some([0x1234, 0, 0, 0]);
    assert!(
        DataCell::with_params(vec!(), &[0; 1024], CellType::Big, 0, None, hashes, depths).is_err()
    );
    ...
    //     - exotic - always **1**
    //     - refs count - always **5 -** actually refs 0, 5 is a label to identify a
    //       big cell by the first byte
    ...
    let mut testcase = |len| -> Result<()> {
        ...
        let cell1 =
            Cell::with_cell_impl(DataCell::with_raw_data(vec![], buf[offset..].to_vec(), None)?);
        let cell2 = Cell::with_cell_impl(DataCell::with_external_data(
            vec![],
            &Arc::new(buf.clone()),
            offset,
            None,
