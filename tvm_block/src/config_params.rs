...
impl ConfigParams {
    ...
    pub fn important_config_parameters_changed(
        ...
        // at least the parameters affecting the computations of validator sets
...
impl GasLimitsPrices {
    ...
    pub fn calc_gas_fee(&self, gas_used: u64) -> u128 {
        // There is a flat_gas_limit value which is the minimum gas value
        // possible and has fixed price. If actual gas value is less
        // then flat_gas_limit then flat_gas_price paid. If actual gas
        // value is bigger then flat_gas_limit then flat_gas_price paid
        // for first flat_gas_limit gas and remaining value costs
        // gas_price
        if gas_used <= self.flat_gas_limit {
            self.flat_gas_price as u128
        } else {
            // gas_price is pseudo value (shifted by 16 as forward and storage
            // price) after calculation divide by 0xffff with ceil
            // rounding
            self.flat_gas_price as u128
