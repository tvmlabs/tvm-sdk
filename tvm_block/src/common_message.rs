...
impl CommonMessage {
    ...
    fn unexpected_variant_error(&self, expected: &str) -> Error {
        error!(BlockError::UnexpectedStructVariant(expected.to_string(), self.get_type_name()))
    }
}
    ...
    fn read_from_with_opts(&mut self, slice: &mut SliceData, opts: u8) -> Result<()> {
        ...
        if opts & SERDE_OPTS_COMMON_MESSAGE != 0 {
            ...
            *self = match tag {
            ...
            fail!(BlockError::UnsupportedSerdeOptions("CommonMessage".to_string(), opts as usize));
        }
        Ok(())
    }
}
