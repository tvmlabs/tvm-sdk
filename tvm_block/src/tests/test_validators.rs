...
fn test_fast_finality_roles() {
    ...
    let descr = ShardDescr { seq_no: 1, ..Default::default() };
    let mut shards_tree = BinTree::with_item(&descr).unwrap();
    shards_tree
        .split(SliceData::with_bitstring(vec![0], 0), |d| Ok((d.clone(), d.clone())))
        .unwrap();
    shards_tree
        .split(SliceData::with_bitstring(vec![0b0000_0000], 1), |d| Ok((d.clone(), d.clone())))
        .unwrap();
    shards_tree
        .split(SliceData::with_bitstring(vec![0b1000_0000], 1), |d| Ok((d.clone(), d.clone())))
        .unwrap();
    shards_tree
        .split(SliceData::with_bitstring(vec![0b0000_0000], 2), |d| Ok((d.clone(), d.clone())))
        .unwrap();
    shards_tree
        .split(SliceData::with_bitstring(vec![0b0100_0000], 2), |d| Ok((d.clone(), d.clone())))
        .unwrap();
    shards_tree
        .split(SliceData::with_bitstring(vec![0b1000_0000], 2), |d| Ok((d.clone(), d.clone())))
        .unwrap();
    shards_tree
        .split(SliceData::with_bitstring(vec![0b1100_0000], 2), |d| Ok((d.clone(), d.clone())))
        .unwrap();
    ...
    for _mc_seqno in 0..10_000 {
        ...
        for ident in idents.iter() {
            ...
            if init {
                ...
                for i in 0..VALIDATORS_COUNT {
                    if i != collators.current.collator {
                        collators
                            .stat
                            .update(i as u16, |f| f.saturating_sub(ff_config.familiarity_fading))
                            .unwrap();
                    }
                }
            }
            ...
            if collators.current.finish < descr.seq_no || problem {
                ...
                let black_list = if init { vec![] } else { vec![collators.current.collator] };
                ...
                .unwrap();
                // println!("find_validator_for_role time: {}micros",
        ...
        for validator in 0..(common_stat.values.len() as u16) {
            ...
            common_stat
                .update(validator, |unreliability| unreliability.saturating_sub(fading))
                .unwrap();
        }
...
fn test_fast_finality_roles_2() -> Result<()> {
    ...
    for _ in 0..10 {
        ...
        let result = crate::CollatorRange { collator, mempool, start, finish };
