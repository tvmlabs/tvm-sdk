...
fn test_serde_mesh() {
    ...
    assert!(matches!(CommonMessage::construct_from_cell(cell.clone()), Err(_)));
    let msg2 =
        CommonMessage::construct_from_cell_with_opts(cell, SERDE_OPTS_COMMON_MESSAGE).unwrap();
...
fn test_childcell_new_format() {
    ...
    assert!(matches!(CommonMessage::construct_from_cell(msgcell.cell()), Err(_)));
    assert!(matches!(Message::construct_from_cell(msgcell.cell()), Err(_)));
...
fn test_childcell_old_format() {
    let msgcell = ChildCell::with_struct(&CommonMessage::Std(Message::default())).unwrap();
    let msg1 = msgcell.read_struct().unwrap();
    assert!(matches!(msg1, CommonMessage::Std(_)));
