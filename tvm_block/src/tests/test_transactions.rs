...
fn test_account_block_serde_mesh() {
    ...
    let mut account_block =
        generate_account_block(address.clone(), 32, SERDE_OPTS_COMMON_MESSAGE).unwrap();
    for _ in 0..10 {
        let transaction =
            generate_transaction_with_opts(address.clone(), SERDE_OPTS_COMMON_MESSAGE);
...
fn test_shard_account_blocks_serde_empty() {
    ...
    let shard_block = generate_test_shard_account_block(SERDE_OPTS_EMPTY);
    assert!(matches!(
        write_read_and_assert_with_opts(shard_block, SERDE_OPTS_COMMON_MESSAGE),
        Err(_)
    ));
}
...
fn test_transaction_with_common_message() {
    ...
    let tr2 = Transaction::construct_from_cell_with_opts(cell.clone(), SERDE_OPTS_COMMON_MESSAGE)
        .unwrap();
...
pub fn generate_transaction_with_opts(address: AccountId, opts: u8) -> Transaction {
    ...
    let mut tr = if opts & SERDE_OPTS_COMMON_MESSAGE != 0 {
        ...
        Transaction::with_address_and_status(address, data.orig_status.clone())
    };
    tr.write_in_msg(Some(&data.in_msg)).unwrap();
