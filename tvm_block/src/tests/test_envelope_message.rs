...
fn test_serialize_intermediate_addr_regular() {
    check_serialization_intermediate_addr_regular(
        IntermediateAddressRegular::with_use_src_bits(0).unwrap(),
    );
    check_serialization_intermediate_addr_regular(
        IntermediateAddressRegular::with_use_src_bits(1).unwrap(),
    );
    check_serialization_intermediate_addr_regular(
        IntermediateAddressRegular::with_use_src_bits(54).unwrap(),
    );
    check_serialization_intermediate_addr_regular(
        IntermediateAddressRegular::with_use_src_bits(96).unwrap(),
    );
}
...
fn test_serialize_intermediate_addr_simple() {
    check_serialization_intermediate_addr_simple(IntermediateAddressSimple::with_addr(
        -1,
        0x0102030405060708,
    ));
    check_serialization_intermediate_addr_simple(IntermediateAddressSimple::with_addr(
...
fn test_serialize_intermediate_addr_ext() {
    check_serialization_intermediate_addr_ext(IntermediateAddressExt::with_addr(
        -1,
        0x0102030405060708,
    ));
    check_serialization_intermediate_addr_ext(IntermediateAddressExt::with_addr(
...
fn test_serialize_inpermediate_address() {
    check_serialization_intermediate_addr(IntermediateAddress::Regular(
        IntermediateAddressRegular::with_use_src_bits(0).unwrap(),
    ));
    check_serialization_intermediate_addr(IntermediateAddress::Regular(
        IntermediateAddressRegular::with_use_src_bits(96).unwrap(),
    ...
    check_serialization_intermediate_addr(IntermediateAddress::Simple(
        IntermediateAddressSimple::with_addr(-1, 0x0102030405060708),
    ));
    check_serialization_intermediate_addr(IntermediateAddress::Simple(
        IntermediateAddressSimple::with_addr(1, 0xFE_FE_FE_FE_FE_FE_FE_FE),
    ));

    check_serialization_intermediate_addr(IntermediateAddress::Ext(
        IntermediateAddressExt::with_addr(-1, 0x0102030405060708),
    ));
    check_serialization_intermediate_addr(IntermediateAddress::Ext(
        IntermediateAddressExt::with_addr(1, 0xCD_CD_CD_CD_CD_CD_CD_CD),
    ));
}
