...
fn get_out_ext_msg() -> OutMsg {
    ...
    let msg_cell = ChildCell::with_struct(&CommonMessage::Std(get_message())).unwrap();
...
fn test_out_msg_dequeue_serialization() {
    let mut b = BuilderData::new();
    let out_msg = OutMsg::Dequeue(OutMsgDequeue::with_cells(
        ChildCell::with_struct(&MsgEnvelope::default()).unwrap(),
        243563457456709,
    ));
...
fn test_out_msg_dequeue_short_serialization() {
    let mut b = BuilderData::new();
    let out_msg = OutMsg::DequeueShort(OutMsgDequeueShort {
...
fn test_work_with_out_msg_desc() {
    ...
    let msg =
        CommonMessage::Std(get_message_with_addrs(create_account_id(1), create_account_id(2)));
    ...
    let msg =
        CommonMessage::Std(get_message_with_addrs(create_account_id(2), create_account_id(1)));
    let out_msg_ext = OutMsg::external(ChildCell::with_struct(&msg).unwrap(), tr_cell.clone());
    ...
    let msg =
        CommonMessage::Std(get_message_with_addrs(create_account_id(3), create_account_id(4)));
    let msg_in = InMsg::external(ChildCell::with_struct(&msg).unwrap(), tr_cell.clone());

    let env = MsgEnvelope::with_message_and_fee(&msg.get_std().unwrap(), Grams::one()).unwrap();
    ...
    let msg =
        CommonMessage::Std(get_message_with_addrs(create_account_id(5), create_account_id(6)));
    let msg_in = InMsg::external(ChildCell::with_struct(&msg).unwrap(), tr_cell.clone());
...
fn test_out_msg_queue_and_info() {
    ...
    let omq_info =
        OutMsgQueueInfo::with_params(queue, ProcessedInfo::default(), IhrPendingInfo::default());
...
fn test_outmsgdescr_common_msg() {
    ...
    let enveloped = MsgEnvelope::with_common_msg_support(&cmn_std_msg, 1.into()).unwrap();
    ...
    let out_msg = OutMsg::new(
        ChildCell::with_struct_and_opts(&enveloped, SERDE_OPTS_COMMON_MESSAGE).unwrap(),
    ...
    let descr = OutMsgDescr::construct_from_cell_with_opts(cell.clone(), SERDE_OPTS_COMMON_MESSAGE)
        .unwrap();
    let mut msg = None;
    let _ = descr
        .iterate_objects(|x| {
    ...
    for opts in [SERDE_OPTS_COMMON_MESSAGE, SERDE_OPTS_EMPTY] {
        ...
        let enveloped = match opts {
            SERDE_OPTS_COMMON_MESSAGE => {
                MsgEnvelope::with_common_msg_support(&msg, 10.into()).unwrap()
            }
            SERDE_OPTS_EMPTY => {
                MsgEnvelope::with_message_and_fee(&msg.get_std().unwrap(), 10.into()).unwrap()
            }
            _ => unreachable!(),
        };
