...
pub enum InMsg {
    ...
    /// msg_import_imm$011 in_msg:^MsgEnvelope transaction:^Transaction
    /// fwd_fee:Grams = InMsg;
    Immediate(InMsgFinal),
    /// Immediately routed internal messages
    /// msg_import_fin$100 in_msg:^MsgEnvelope transaction:^Transaction
    /// fwd_fee:Grams = InMsg;
    Final(InMsgFinal),
...
impl InMsg {
    /// Create external
    pub fn external(msg_cell: ChildCell<CommonMessage>, tr_cell: ChildCell<Transaction>) -> InMsg {
    ...
    /// Transaction exist only in External, IHR, Immediate and Final inbound
...
impl Serializable for InMsg {
    ...
    fn write_with_opts(&self, cell: &mut BuilderData, opts: u8) -> Result<()> {
        let (opt_bits, len) =
            if opts == SERDE_OPTS_COMMON_MESSAGE { (COMMON_MSG_TAG, 8) } else { (0, 3) };
        match self {
...
impl Deserializable for InMsg {
    ...
    fn read_from_with_opts(&mut self, cell: &mut SliceData, opts: u8) -> Result<()> {
        ...
        *self = match tag {
            ...
            tag => {
                fail!(BlockError::InvalidConstructorTag { t: tag as u32, s: "InMsg".to_string() })
            }
        };
        Ok(())
    }
