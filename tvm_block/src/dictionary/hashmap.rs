...
/// If key length is less than bit_len it should be filled by zeros on the left
/// <- TODO:
...
impl HashmapE {
    ...
    pub fn split(&self, key: &SliceData) -> Result<(Self, Self)> {
        self.hashmap_split(key).map(|(left, right)| {
            (Self::with_hashmap(self.bit_len, left), Self::with_hashmap(self.bit_len, right))
        })
    }
    ...
    // pub fn subtree_with_prefix(&self, prefix: &SliceData, gas_consumer: &mut dyn
    // GasConsumer) -> Result<Self> {     self.subtree_by_prefix(prefix,
    // gas_consumer) }
    ...
    ($varname:ident, $bit_len:expr, $x_type:ty) => {
        ...
        impl $varname {
            ...
            pub const fn new() -> Self {
                Self($crate::HashmapE::with_hashmap($bit_len, None), $crate::SERDE_OPTS_EMPTY)
            }
                ...
                F: FnMut($x_type) -> Result<bool>,
            {
                $crate::HashmapType::iterate_slices(&self.0, |_, ref mut slice| {
                    p(<$x_type>::construct_from_with_opts(slice, self.1)?)
                })
            }
                ...
                F: FnMut(K) -> Result<bool>,
            {
                $crate::HashmapType::iterate_slices(&self.0, |ref mut key, _| {
                    p(K::construct_from_with_opts(key, self.1)?)
                })
            }
                ...
                F: FnMut(K, $x_type) -> Result<bool>,
            {
                $crate::HashmapType::iterate_slices(&self.0, |ref mut key, ref mut slice| {
                    p(K::construct_from(key)?, <$x_type>::construct_from_with_opts(slice, self.1)?)
                })
            }
            ...
            pub fn get<K: Serializable>(&self, key: &K) -> Result<Option<$x_type>> {
                self.get_as_slice(key)?
                    .map(|ref mut slice| <$x_type>::construct_from_with_opts(slice, self.1))
                    .transpose()
            }
            ...
            pub fn split(&self, split_key: &SliceData) -> Result<(Self, Self)> {
                self.0
                    .split(split_key)
                    .map(|(left, right)| (Self(left, self.1), Self(right, self.1)))
            }
