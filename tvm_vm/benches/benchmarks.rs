...
// Run  `cargo bench -p tvm_vm --bench benchmarks wasmtls`
fn bench_wasmtls(c: &mut Criterion) {
    c.bench_function("wasmtls", |b| {
        let mut total_duration = Duration::default();
        b.iter_custom(|iters| {
            // b.iter(|| {
            //+++++ Let's say I want to measure execution time from this point.
...
// Run  `cargo bench -p tvm_vm --bench benchmarks wasmcalc`
fn bench_wasmcalc(c: &mut Criterion) {
    c.bench_function("wasmcalc", |b| {
        let mut total_duration = Duration::default();
        b.iter_custom(|iters| {
            // b.iter(|| {
            //+++++ Let's say I want to measure execution time from this point.
...
// Run  `cargo bench -p tvm_vm --bench benchmarks rsamod`
fn bench_rsamod(c: &mut Criterion) {
    c.bench_function("rsamod", |b| {
        let mut total_duration = Duration::default();
        b.iter_custom(|iters| {
            // b.iter(|| {
            //+++++ Let's say I want to measure execution time from this point.
            ...
            for _i in 0..iters {
                ...
                //println!("rsa encrypt result is : {:?}", result);
