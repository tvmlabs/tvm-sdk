...
fn load_boc(filename: &str) -> tvm_block::Cell {
    let bytes = read_boc(filename);
    tvm_block::read_single_root_boc(bytes).unwrap()
}
...
fn bench_load_boc(c: &mut Criterion) {
    ...
    c.bench_function("load-boc", |b| {
        b.iter(|| tvm_block::read_single_root_boc(bytes.clone()).unwrap())
    });
}
...
fn bench_ed25519_verify(c: &mut Criterion) {
    ...
    for size in [1, 2, 4, 8, 16, 32, 48, 64, 96, 128] {
        ...
        data.truncate(size);
        let signature = tvm_block::ed25519_sign_with_secret(&secret, &data).unwrap();
    ...
    for input in &signed {
        g.bench_with_input(format!("{}", input.0.len()), input, |b, input| {
            b.iter(|| tvm_block::ed25519_verify(&public, &input.0, &input.1).unwrap())
        });
    }
    g.finish();
}
...
fn bench_chksignu(c: &mut Criterion) {
    ...
    c.bench_function("chksignu/empty", |b| {
        b.iter(|| {
            let mut engine = Engine::with_capabilities(DEFAULT_CAPABILITIES).setup_with_libraries(
                SliceData::load_cell(tvm_block::Cell::default()).unwrap(),
                None,
                Some(stack.clone()),
                None,
                vec![],
    ...
    c.bench_function("chksignu/revised", |b| {
        b.iter(|| {
            ...
            assert_eq!(
                tvm_vm::error::tvm_exception_code(&res.unwrap_err()).unwrap(),
                tvm_block::ExceptionCode::OutOfGas
            );
        })
    });
}
