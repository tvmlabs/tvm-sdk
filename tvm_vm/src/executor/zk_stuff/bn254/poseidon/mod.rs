...
// fn poseidon_test() {
// let input1 =
// Fr::from_str("134696963602902907403122104327765350261").unwrap(); let input2
// = Fr::from_str("17932473587154777519561053972421347139").unwrap(); let input3
// = Fr::from_str("10000").unwrap(); let input4 = Fr::from_str(
...
// 18586133768512220936620570745912940619677854269274689475585506675881198879027
// in decimal let expected =
// hex::decode("
// 33018202c57d898b84338b16d1a4960e133c6a4d656cfec1bd62a9ea00611729") .unwrap();
// assert_eq!(hash.as_slice(), &expected);
//
// 7853200120776062878684798364095072458815029376092732009249414926327459813530
// in decimal let inputs: Vec<Vec<u8>> = vec![vec![1u8], vec![2u8]];
// let hash = poseidon_bytes(&inputs).unwrap();
// let expected =
// hex::decode("
// 9a1817447a60199e51453274f217362acfe962966b4cf63d4190d6e7f5c05c11") .unwrap();
// assert_eq!(hash.as_slice(), &expected);
//
...
// #[cfg(test)]
// lazy_static! {
// static ref POSEIDON_ARK: poseidon_ark::Poseidon =
// poseidon_ark::Poseidon::new(); }
//
// proptest::proptest! {
// #[test]
// fn test_against_poseidon_ark(r in collection::vec(<[u8; 32]>::arbitrary(),
// 1..16)) {
//
// let inputs = r.into_iter().map(|ri|
// ark_bn254::Fr::from_le_bytes_mod_order(&ri)).collect::<Vec<_>>();
// let expected =
// POSEIDON_ARK.hash(inputs.clone()).unwrap().into_bigint().to_bytes_le();
//
// let actual = poseidon_bytes(&inputs.iter().map(|i|
// i.into_bigint().to_bytes_le().to_vec()).collect::<Vec<_>>()).unwrap();
