...
//
// Test-only fixture material (`DARK_DEX_W8_VK_BYTES`,
// `dark_dex_w8_config_params`) lives behind `#[cfg(test)]` so it does
// not bloat the runtime binary.
...
/// Test-only fixture: serialised VK bytes (842 bytes) for the DarkDex W=8
/// circuit. Used by `tvm_vm/src/tests/test_halo2_with_vk.rs` to validate
/// the opcode end to end against a real Halo2 SHPLONK proof. Not used at
/// runtime — the live opcode reads the VK from the caller-supplied bundle.
#[cfg(test)]
pub(crate) const DARK_DEX_W8_VK_BYTES: [u8; 842] = [
