...
use tvm_block::aggregate_and_verify;
use tvm_block::aggregate_public_keys_and_verify;
use tvm_block::aggregate_pure_bls_signatures;
use tvm_block::g1_add;
use tvm_block::g1_in_group;
use tvm_block::g1_mul;
use tvm_block::g1_multiexp;
use tvm_block::g1_neg;
use tvm_block::g1_sub;
use tvm_block::g1_zero;
use tvm_block::g2_add;
use tvm_block::g2_in_group;
use tvm_block::g2_mul;
use tvm_block::g2_multiexp;
use tvm_block::g2_neg;
use tvm_block::g2_sub;
use tvm_block::g2_zero;
use tvm_block::map_to_g1;
use tvm_block::map_to_g2;
use tvm_block::pairing;
use tvm_block::verify;
use tvm_block::ExceptionCode;
use tvm_block::GlobalCapabilities;
use tvm_block::Result;
use tvm_block::SliceData;
use tvm_block::BLS_G1_LEN;
use tvm_block::BLS_G2_LEN;
use tvm_block::BLS_PUBLIC_KEY_LEN;
use tvm_block::BLS_SCALAR_LEN;
use tvm_block::BLS_SIG_LEN;
...
/// Checks aggregated BLS signature for key-message pairs `pk_1 msg_1...pk_n
/// msg_n`. Return true on success, false otherwise. Return false if `n=0`
    ...
    G: Fn(i64) -> i64,
{
    ...
    for i in 0..n {
        ...
        let (_, s) = s.div::<Signaling>(&R, Round::FloorToNegativeInfinity)?;
        scalars.push(
            s.as_builder::<UnsignedIntegerLittleEndianEncoding>(BLS_SCALAR_LEN * 8)?
                .data()
                .try_into()?,
        );
...
/// BLS_G2_MUL ( x s - x*s)
/// Multiplies G2 point `x` by scalar `s`.<br/>Any `s` is valid, including
/// negative.
...
/// Given G1 points `x_i` and G2 points `y_i`, calculates and multiply pairings
/// of `x_i, y_i`. Returns true if the result is the multiplicative identity in
/// FP12, false otherwise. Returns false if `n=0`.
