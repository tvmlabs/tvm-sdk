...
mod tests {
    ...
    // zk_login_inputs.get_kid().to_string()); println!("kid = {}", kid);
    // println!("all_jwk = {:?}", all_jwk);
    //
    // let jwk = all_jwk
    ...
    // .map_err(|_| {
    // ZkCryptoError::GeneralError("Invalid Base64 encoded jwk
    // modulus".to_string()) })
    // .unwrap();
    //
    // let proof = &zk_login_inputs.get_proof().as_arkworks().unwrap();
    // let public_inputs =
    // &[zk_login_inputs.calculate_all_inputs_hash(&eph_pubkey, &modulus,
    // max_epoch).unwrap()];
    //
    ...
    // println!("public_inputs_as_bytes len : {:?}",
    ...
    // prepare_proof_and_public_key_cells_for_stack(eph_pubkey, zk_login_inputs,
    // all_jwk);
    //
    // let verification_key_id: u32 = 0;
    //
    ...
    // test_case_with_refs(code.as_str(), vec![proof_cell.clone(),
    // public_inputs_cell.clone()]) .expect_success();
