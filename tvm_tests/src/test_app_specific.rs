...
use crate::test_framework::create;
use ed25519::signature::Signer;

use rand::Rng;
//use similar::DiffableStr;
use tvm_block::{
    GlobalCapabilities, MsgAddressInt, Serializable, ACTION_CHANGE_LIB, ACTION_COPYLEFT,
...
use crate::test_framework::{expect_exception, test_case, test_case_with_refs, expect_exception_with_capability};
use crate::test_framework::test_case_with_ref;
...
fn test_chksigns_real() {
    ...
    test_case_with_refs("
        ...
        PLDU 256
        CHKSIGNS
    ", vec![/*hash.into_cell().unwrap()*/test_cell, signature.into_cell(), pub_key.into_cell().unwrap()])
...
fn test_chksignu_real() {
    ...
    //sign hash of data cell
    let signature = pair.sign(cell_hash.as_slice()).to_bytes().to_vec();
...
fn call_chksignu(hash_primitive: &str, push_primitive: &str) {
    ...
    let signature = pair.sign(&hash_bytes).to_bytes().to_vec();
    let signature = BuilderData::with_raw(
        signature,
        ed25519_dalek::SIGNATURE_LENGTH * 8
    ).unwrap();
...
fn cell_datasize_quite_normal() {
    test_case_with_ref("
            ...
            TEN
            CDATASIZEQ
        ", crate::test_zk::prepare_cell()).expect_int_stack(&[3, 24, 3, -1]);
